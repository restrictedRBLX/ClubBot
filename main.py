...
def LogMessage(Moderator, Victim, Action, Reason):
    ...
    Embed = discord.Embed(title=Title, description=Description, type="rich", color = 0x8A2BE2)
...
async def Mute(From, Victim, Reason):
    ...
    await Bot.send_message(GetChannel(Victim.server, "mod-logs"), embed=Embeded)
